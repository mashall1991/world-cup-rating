...
import {
  ...
  formatTeamName, getPredictionResultText, getPredictionStats, renderOddsText
} from "../lib/engine.js";
import { openCompare } from "../lib/ui.js";

const cards = computed(() => {
  // 依赖 liveSchedule / odds / teams 的变化
  ...
  return getFeaturedBannerMatches().map((local) => {
    ...
    if (teamA && teamB && local.status === "FINISHED") {
      predictText = getPredictionResultText({ ...local, score: local.liveScore, resultFinal: true }, teamA, teamB);
    }
    return {
